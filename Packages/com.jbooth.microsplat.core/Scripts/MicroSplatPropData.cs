...
#if __MICROSPLAT_MEGA__ && __MICROSPLAT_GENERATEMEGA__
    public const int sMaxTextures = 256;
#else
      ...
      public Texture2D GetTexture()
      {
         ...
         if (propTex == null)
         {
            ...
            {
               propTex = new Texture2D(sMaxTextures, sMaxAttributes, TextureFormat.RGBAFloat, false, true);
            }
            else if (SystemInfo.SupportsTextureFormat(TextureFormat.RGBAHalf))
            {
               propTex = new Texture2D(sMaxTextures, sMaxAttributes, TextureFormat.RGBAHalf, false, true);
            }
            else
            {
               ...
               propTex = new Texture2D(sMaxTextures, sMaxAttributes, TextureFormat.RGBA32, false, true);
            }
